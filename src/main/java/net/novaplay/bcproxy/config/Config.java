package net.novaplay.bcproxy.config;
import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.reflect.TypeToken;

import net.novaplay.bcproxy.scheduler.FileWriteTask;
    ...
    public boolean load(String file, int type, ConfigSection defaultMap) {
        ...
        if (!this.file.exists()) {
            try {
                ...
                Server.getInstance().getLogger().error("Could not create Config " + this.file.toString()+"\n\r"+Utils.getExceptionMessage(e));
            }
        ...
        for (String line : content.split("\n")) {
            if (Pattern.compile("[a-zA-Z0-9\\-_\\.]*+=+[^\\r\\n]*").matcher(line).matches()) {
                ...
                if (this.config.containsKey(k)) {
                    Server.getInstance().getLogger().debug("[Config] Repeated property " + k + " on file " + this.file.toString());
                }
                switch (v_lower) {
                    case "on":
                    case "true":
